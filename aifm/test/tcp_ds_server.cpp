...
// 所做的工作只需要应答，根据object_id来找到数据，把数据传输回去
// 1. 首先程序和各个node之间要建立一个连接pool，每次建立连接要数据的时候要取出一个连接
  ...
  // shared_pool_.push(remote_slave);//用完之后放回去
// 2. 先读请求，找到各个关键字，再读取对象
// firstly, tcp_read_until()
// 3. 把数据传输回去
// then, tcp_write_until()
// Request:
// |Opcode = KOpReadObject(1B) | ds_id(1B) | obj_id_len(1B) | obj_id |
// Response:
// |data_len(2B)|data_buf(data_len B)|
void process_read_object(tcpconn_t *c) {
  uint8_t req[Object::kDSIDSize + Object::kIDLenSize + Object::kMaxObjectIDSize];
